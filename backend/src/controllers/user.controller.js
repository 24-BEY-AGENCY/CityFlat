...
export async function addToWishlist(req, res) {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Apartment added to wishlist successfully" });
  } catch (error) {
...
export function httpRemoveFromWishlist(req, res) {
  if (!validationResult(req).isEmpty()) {
    ...
    const userId = req.user.id;
    userDb
      .findByIdAndUpdate(userId, { $pull: { wishlist: apartmentId } })
      .then(() => {
        res
          .status(200)
          .json({ message: "Apartment removed from wishlist successfully!" });
      })
      .catch((err) => res.status(500).json({ error: err.message }));
  }
}
