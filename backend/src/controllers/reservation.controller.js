...
export async function httpCreateReservationPaypal(order, req, res) {
  console.log("creating a reservation PayPal !!!");
  try {
...
export async function httpGetAcceptedOrdersForUser(req, res) {
  try {
    const userId = req.user.id;
    console.log(userId);

    const orders = await orderDb
      .find({ User: userId, state: "ACCEPTED" })
...
export async function httpGetAcceptedAndPaidOrdersForUser(req, res) {
  try {
    const userId = req.user.id;

    const orders = await orderDb
      .find({ User: userId, state: "ACCEPTED", isPaied: true })
